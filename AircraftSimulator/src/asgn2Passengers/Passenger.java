...
public abstract class Passenger {
	...
	public Passenger(int bookingTime, int departureTime) throws PassengerException  {
		...
		if (bookingTime < 0) {
			...
			throw new PassengerException("Unable to construct passenger - Departure time cannot be less than or equal to zero");
		}else if (departureTime < bookingTime){
			throw new PassengerException("Unable to construct passenger - Departure time cannot be less than Booking time");
		}
	...
	public void cancelSeat(int cancellationTime) throws PassengerException {
		...
		if (this.newState){
			throw new PassengerException("Unable to perform cancelSeat operation - passenger is in new state");
		} else if (this.inQueue){
			throw new PassengerException("Unable to perform cancelSeat operation - passenger is in queued state");
		} else if (this.flown){
			throw new PassengerException("Unable to perform cancelSeat operation - passenger is in flown state");
		} else if (this.refused){
			throw new PassengerException("Unable to perform cancelSeat operation - passenger is in refused state");
		} else if(!this.confirmed){
			// Something unexpected has happened - error in state transition logic
			throw new PassengerException("Unable to perform cancelSeat operation - passenger is in unknown state");
		}	
		
		// Check that cancellation time is correct
		if(cancellationTime < 0){
			throw new PassengerException("Unable to perform cancelSeat operation - Cancellation time cannot be less than zero");
		} else if (departureTime < cancellationTime){
			throw new PassengerException("Unable to perform cancelSeat operation - Departure time cannot be less than Cancellation time");
		}
	...
	public void confirmSeat(int confirmationTime, int departureTime) throws PassengerException {
		// Check if in correct state
		if (this.confirmed){
			throw new PassengerException("Unable to perform confirmSeat operation - passenger is in confirmed state");
		} else if (this.flown){
			throw new PassengerException("Unable to perform confirmSeat operation - passenger is in flown state");
		} else if (this.refused){
			throw new PassengerException("Unable to perform confirmSeat operation - passenger is in refused state");
		} else if(!this.newState ^ this.inQueue){ // ^ is xor
			// Something unexpected has happened - error in state transition logic
			throw new PassengerException("Unable to perform confirmSeat operation - passenger is in unknown state");
		}	
		
		// Check that cancellation time is correct
		if(confirmationTime < 0){
			throw new PassengerException("Unable to perform confirmSeat operation - Confirmation time cannot be less than zero");
		} else if (departureTime < confirmationTime){
			throw new PassengerException("Unable to perform confirmSeat operation - Departure time cannot be less than Confirmation time");
		}
	...
	public void flyPassenger(int departureTime) throws PassengerException {
		// Check if in correct state
		if (this.newState){
			throw new PassengerException("Unable to perform flyPassenger operation - passenger is in new state");
		} else if (this.inQueue){
			throw new PassengerException("Unable to perform flyPassenger operation - passenger is in queued state");
		} else if (this.flown){
			throw new PassengerException("Unable to perform flyPassenger operation - passenger is in flown state");
		} else if (this.refused){
			throw new PassengerException("Unable to perform flyPassenger operation - passenger is in refused state");
		} else if(!this.confirmed){
			// Something unexpected has happened - error in state transition logic
	...
	public void queuePassenger(int queueTime, int departureTime) throws PassengerException {
		// Check if in correct state
		if (this.confirmed){
			throw new PassengerException("Unable to perform queuePassenger operation - passenger is in confirmed state");
		} else if (this.inQueue){
			throw new PassengerException("Unable to perform queuePassenger operation - passenger is in queued state");
		} else if (this.flown){
			throw new PassengerException("Unable to perform queuePassenger operation - passenger is in flown state");
		} else if (this.refused){
			throw new PassengerException("Unable to perform queuePassenger operation - passenger is in refused state");
		} else if(!this.newState){
			// Something unexpected has happened - error in state transition logic
			throw new PassengerException("Unable to perform queuePassenger operation - passenger is in unknown state");
		}
		
		// Check that cancellation time is correct
		if(queueTime < 0){
			throw new PassengerException("Unable to perform queuePassenger operation - Queue time cannot be less than zero");
		}  else if (departureTime < queueTime){
			throw new PassengerException("Unable to perform queuePassenger operation - Departure time cannot be less than Queue time");
		}
	...
	public void refusePassenger(int refusalTime) throws PassengerException {
		// Check if in correct state
		if (this.confirmed){
			throw new PassengerException("Unable to perform refusePassenger operation - passenger is in new state");
		} else if (this.flown){
			throw new PassengerException("Unable to perform refusePassenger operation - passenger is in flown state");
		} else if (this.refused){
			throw new PassengerException("Unable to perform refusePassenger operation - passenger is in refused state");
		} else if(!this.newState ^ this.inQueue){
			// Something unexpected has happened - error in state transition logic
			throw new PassengerException("Unable to perform refusePassenger operation - passenger is in unknown state");
		}
		
		// Check that cancellation time is correct
		if(refusalTime < 0){
			throw new PassengerException("Unable to perform refusePassenger operation - Resfusal time cannot be less than zero");
		}  else if (refusalTime < bookingTime){
			throw new PassengerException("Unable to perform refusePassenger operation - Resfusal time cannot be less than Booking time");
		}
